...
// fim das configurações de tempo inativo//
    ...
    function redirect($msg, $url, $msgPerso = null) {
        $_SESSION['MSG'] = $msg;
        if ($msgPerso != null) {
             $_SESSION['MSGPERSON']=$msgPerso;
        } 
        echo $URL = RAIZ . $url;
        header("location: $URL");
    }
...
//Função para Padronizar Nome
    ...
    function retornoIdUF($parametro, $dado) {
        //$parametro==1 = id que vai retornar uf --- $parametro==2 = UF que vai retornar id

        if ($parametro == 1) {
             $consulta = $this->consultaWhere('estado', 'id_estado', $dado);
            $dados = mysqli_fetch_array($consulta);
            return $dados['str_uf'];
        } elseif ($parametro == 2) {
            $consulta = $this->consultaWhere('estado', 'str_uf', $dado);
